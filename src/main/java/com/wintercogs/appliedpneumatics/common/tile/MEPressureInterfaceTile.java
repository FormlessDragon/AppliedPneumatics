...
public class MEPressureInterfaceTile extends AENetworkedTile
    ...
    public void serverTick() {
        ...
        if (validateAirHandlerIfPossible() && !this.getWorld().isRemote) {
            this.airHandler.update();
        }
    }

    public void onNeighborChanged() {
        if (validateAirHandlerIfPossible()) {
            this.airHandler.onNeighborChange();
        }
    }

    private boolean validateAirHandlerIfPossible() {
        World world = this.getWorld();
        BlockPos pos = this.getPos();
        if (isAirHandlerBoundToTile(this.airHandler, world, pos)) {
            return true;
        }
        ...
        return isAirHandlerBoundToTile(this.airHandler, world, pos);
    }

    static boolean isAirHandlerBoundToTile(IAirHandler airHandler, World world, BlockPos pos) {
        return airHandler != null && world != null && pos != null
                && airHandler.getWorld() == world
                && pos.equals(airHandler.getPos());
    }
